...
type Props = {
  coworkerId: string;
  coworkerData: CoworkerEditableProps;
}

const EditCoworkerForm = ({coworkerId, coworkerData}: Props) => {
  ...
  const onSubmit = async (formData: CoworkerEditableProps) => {
    ...
    const response = await coworkerService.update(
      coworkerId,
      {
    ...
    if (response instanceof ServiceError) {
      ...
      setSnackbarMessage(t('updateCoworker.success'));
  ...
  return (
    <CoworkerForm
      ...
      currentData={coworkerData}
    />
  )
}

export default EditCoworkerForm;